...
contract Vote{
    ...
    event VoteState(bytes32 date, bytes32 state);
    event AgreeDisagree(bool value);
    ...
    function _getResult() public returns(uint,uint,uint){
        ...
        VoteState(budgets[id].date,budgets[id].state);
        
        return (budgets[id].agree,budgets[id].disagree,total_vote);
    
    }
    
    function _printVote()public view{
